...
def optimize(m, b, nvert, nstrains, g, options):
    M = 1000000
    ...
    f = m.addVars(list(range(nstrains)), lb=1,ub=1, vtype=GRB.CONTINUOUS, name = 'f')
    ...
    z = m.addVars(list(range(nvert*nstrains)), lb=0, vtype=GRB.CONTINUOUS, name='z')

    m.update()

    A = []                                  #Put the alpha and z variables in
    Z = []                                  #matrix form
    for i in range(nvert*nstrains):
        A.append(alpha[i])
        Z.append(z[i])
    A = array([A]).reshape(nvert,nstrains)
    Z = array([Z]).reshape(nvert,nstrains)
    ...
    for i in range(nvert):
        for j in range(nstrains):
            m.addConstr(Z[i,j] <= M*A[i,j])  #Linearize A[i,j]*f[j] by using
            m.addConstr(Z[i,j] <= f[j])      #Z[i,j]
            m.addConstr(Z[i,j] >= f[j]-(1-A[i,j])*M)

        #Sum of the frequencies of strains is the same as the sum of the
        #contigs in a node
        m.addConstr(sum(Z[i,:]) + e1[i] == b[i,0])
    ...
    if m.status == GRB.Status.OPTIMAL:
        for i in reversed(range(m.solcount)):
            a = []                          #Create lists with the frequencies and
            f = []                          #binaries optimized by the model
            ...
            if options['display']:      #If the display option is on, print
                ...
                print('\nObjective value: %g' % m.PoolobjVal) # - the obj value
